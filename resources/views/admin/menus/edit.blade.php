        ...
        <form method='post' action="{{route('menus.update',$menu->id)}}">
            ...
            <div class="row">
                ...
                <div class="col-md-6 col-12 mt-3">
                    ...
                    <input type='text' class="form-control @error('key') is-invalid @enderror" id="key" name="key" value="{{old('key')??$menu->key??''}}" required>
                    @error('key')
                        <span class="text-danger font-12">{{ $message }}</span>
                    @enderror
                </div>
                @include("admin.partials.subscribe-accessibility",["data"=>$menu,"notRefreshTime"=>true])
            </div>
