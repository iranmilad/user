...
@section('content')
        <form method='post' action="{{route('memberLists.store')}}">
            @csrf
            <div class="row">
                ...
                <div class="col-md-6 col-12 mt-3">
                    ...
                    <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" required>{{old('class')??''}}</textarea>
                    @error('description')
                        <span class="text-danger font-12">{{ $message }}</span>
                    @enderror
                </div>
